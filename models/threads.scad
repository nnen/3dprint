...
module DN_thread(diameter=8, pitch=1, length=1, internal=false, n_starts=1)
{
   // Number of turns needed.
   n_turns = floor(length/pitch);
   n_segments = segments(diameter);
   ...
   union() {
      intersection() {
         ...
         for (i=[-1*n_starts : n_turns+1]) {
            translate([0, 0, i*pitch]) {
               DN_thread_turn(diameter, pitch, internal, n_starts);
            }
         }
...
module DN_thread_turn(diameter, pitch, internal, n_starts)
{
   n_segments = segments(diameter);
   fraction_circle = 1.0/n_segments;
   for (i=[0 : n_segments-1]) {
      rotate([0, 0, i*360*fraction_circle]) {
         translate([0, 0, i*n_starts*pitch*fraction_circle]) {
            thread_triangle(diameter/2, pitch, internal, n_starts);
         }
      }
   }
}
...
module thread_triangle(radius, pitch, internal, n_starts)
{
   n_segments = segments(radius*2);
   fraction_circle = 1.0/n_segments;
   ...
   inner_r = 0.95*radius; // Does NOT do Dmin_truncation - do later with
                               // cylinder.
   ...
   */
   ...
   polyhedron(
      points = [
                [-x_incr_outer/2, -outer_r, pitch/4],                        // [0]
                [x_incr_outer/2, -outer_r, z_incr + pitch/4],                    // [1]
                [x_incr_outer/2, -outer_r, pitch*3/4+ z_incr],            // [2]
                [-x_incr_outer/2, -outer_r, pitch*3/4],                    // [3]

                [-x_incr_inner/2, -inner_r, z0_outer],                 // [4]
                [x_incr_inner/2, -inner_r, z0_outer + z_incr],         // [5]
                [x_incr_inner/2, -inner_r, pitch - z0_outer + z_incr], // [6]
                [-x_incr_inner/2, -inner_r, pitch - z0_outer]          // [7]
               ],
